...
mod tests {
    ...
    extern "C" fn pthread_send_sigusr1(_arg: *mut c_void) -> *mut c_void {
    ...
    extern "C" fn pthread_send_sigqueue_usr2(_arg: *mut c_void) -> *mut c_void {
        let tid = TEST_THREAD_TID.load(Ordering::SeqCst) as pid_t;
        let val = sigval {
    ...
    extern "C" fn pthread_deliver_usr1_after_delay(_arg: *mut c_void) -> *mut c_void {
        let tid = TEST_THREAD_TID.load(Ordering::SeqCst) as pid_t;
        usleep(20_000);
        tkill(tid, SIGUSR1);
        ptr::null_mut()
    ...
    fn check_sigset_ops() {
        unsafe {
            TEST_THREAD_TID.store(libc::pthread_self() as usize, Ordering::SeqCst);
    ...
    #[test]
    #[cfg_attr(
        any(target_board = "qemu_riscv32", target_board = "qemu_riscv64"),
        ignore = "flaky on target"
    )]
        ...
        unsafe {
            ...
            assert_eq!(
                pthread_create(
                    &mut tid2 as *mut _,
                    ptr::null(),
                    pthread_deliver_usr1_after_delay,
                    ptr::null_mut()
                ),
                0
            );
